...
public class CookieHttpOnlyFlagDetector extends AbstractCallFlowDetector implements TaintFrameAdditionalVisitor {
    ...
    private static final InvokeMatcherBuilder SET_HTTP_METHOD = invokeInstruction() //
            .atClass("javax/servlet/http/Cookie").atMethod("setHttpOnly").withArgs("(Z)V");

    public CookieHttpOnlyFlagDetector(BugReporter bugReporter) {
        super(bugReporter);
        ...
        for (int taintIndex = 0; taintIndex < fact.getNumSlots(); taintIndex++) {
            try {
                ...
                if (COOKIE_OBJECT_TYPE.equals(fact.getValue(taintIndex).getRealInstanceClassName())
                    ...
                    BugInstance inst = new BugInstance(this, HTTPONLY_COOKIE_TYPE, Priorities.NORMAL_PRIORITY) //
    ...
    public void visitInvoke(InvokeInstruction instruction, ConstantPoolGen cpg, MethodGen methodGen, TaintFrame frameType) {
        ...
        try {
            if (SET_HTTP_METHOD.matches(instruction, cpg) && frameType.getStackValue(0).getIntegerConstantValue().intValue() == TRUE_INT_VALUE) {
